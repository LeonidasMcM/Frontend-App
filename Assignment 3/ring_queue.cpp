...
public:
	...
	ItemType back() const {
		...
		// Replace the line(s) below with your code.
		return buffer[(this->end_index() - 1) % MAX_SIZE];
	}
	...
	void push_back(const ItemType& value) {
		if (ring_size != MAX_SIZE) {
			ring_size++;
			buffer[this->end_index() % MAX_SIZE] = value;
		}
		else {
			buffer[(this->end_index() + 1) % MAX_SIZE];
	...
	void pop_front() {
		// move begin index, since we do not explicitly care about the value anymore
		begin_index = (begin_index + 1) % MAX_SIZE;
	...
	iterator end() {
		...
		if (begin_index < end_index()) {
			...
			int n = ((MAX_SIZE - begin_index) % MAX_SIZE) + end_index();
			return iterator(this, n);
		}
